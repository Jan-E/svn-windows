...
!IF [IF EXIST "$(APR_SRC)\lib\libapr-1.lib" exit 1] == 1
APR_LIBS = "$(APR_SRC)\lib\libapr-1.lib"
!ELSE
...
!IF [IF EXIST "$(APRUTIL_SRC)\lib\libaprutil-1.lib" exit 1] == 1
APRUTIL_LIBS = "$(APRUTIL_SRC)\lib\libaprutil-1.lib"
!ELSE
...
!IF "$(ZLIB_LIBDIR)" == ""
!IF "$(DEBUG_BUILD)" == ""
ZLIB_LIBS = "$(ZLIB_SRC)\lib\zlib.lib"
!ELSE
...
LIB32_OBJS = $(LIB32_OBJS) "$(OPENSSL_SRC)\lib\libcrypto.lib" \
               "$(OPENSSL_SRC)\lib\libssl.lib"
!ELSE
LIB32_OBJS = $(LIB32_OBJS) "$(OPENSSL_SRC)\lib\libcrypto.lib" \
               "$(OPENSSL_SRC)\lib\libssl.lib"
!ENDIF
...
TEST_OBJS = \
    ...
    "$(INTDIR)\serf_bwtp.obj" \
    "$(INTDIR)\serf_get.obj" \
    "$(INTDIR)\serf_request.obj" \
    "$(INTDIR)\serf_response.obj" \
    "$(INTDIR)\serf_server.obj" \
    "$(INTDIR)\serf_spider.obj" \
    "$(INTDIR)\test_all.obj" \
    "$(INTDIR)\test_auth.obj" \
